...
/*   Updated: 2024/04/08 16:16:18 by adurusoy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void hit_cylinder_cap(t_cylinder *cy, t_ray *ray, double height, bool *a)
{
    ...
    root = dot_v3(subtract_v3(circle_center, ray->origin), cy->normal) / dot_v3(ray->direction, cy->normal);
...
bool	calc_cy_roots(t_cylinder *cy, t_ray *ray, double *roots)
{
	...
	half_b = dot_v3(cross_v3(ray->direction, cy->normal), cross_v3(delta, cy->normal));
...
void	hit_cylinder_side(t_cylinder *cy, t_ray *ray, double *roots, bool *a)
{
	...
	while (i < 2)
	{
		if (roots[i] < ray->hit.hit_len && roots[i] > EPSILON)
		{
			...
			ray->hit.hit_point = add_v3(ray->origin, scale_v3(ray->direction, roots[i]));
			ray->hit.normal = normalize(subtract_v3(ray->hit.hit_point, add_v3(cy->cordnts, scale_v3(cy->normal, hit_height))));
			ray->hit.color = cy->color;
			ray->hit.type = CYLINDER;
			*a = true;
		}
